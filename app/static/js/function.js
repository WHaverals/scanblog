(function ($) {
	...
	if($('#style-screenshot').length){
		var screenshot = new Swiper('#style-screenshot', {
			autoplay: false,
			slidesPerView: 8,
			spaceBetween: 30,
		...
		$("#style-screenshot .swiper-slide").on('click touch', function () {
			var $src = $(this).data('mfp-src');
			if($src!=''){
				$('#showcase-image img').attr("src",$src);
			}
		});
	}
	...
	if ($window.width() < 768) {
		...
		$('#style-screenshot .swiper-slide').magnificPopup({
		  type: 'image',
		  gallery:{
			enabled:true
		  }
